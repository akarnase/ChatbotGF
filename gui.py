...
def find_answer(user_input, qa_df):
    if qa_df is not None:
        # Standardize column names (optional, if column names might have different cases)
        qa_df.columns = qa_df.columns.str.strip().str.lower()  # Remove spaces and convert to lowercase

        # Check if the "question" column exists
        if "question" in qa_df.columns:
            # Convert both the user input and the "question" column to lowercase for case-insensitive matching
            user_input = user_input.lower().strip()  # Standardize user input
            qa_df["question"] = qa_df["question"].str.lower().str.strip()  # Standardize the "question" column
